...
select target_dag.project_id,
    ...
    inner join redcap_data_access_groups target_dag on target_dag.group_name = source_dag.group_name
        where source_dag.project_id = 16
          and target_dag.project_id = 57
          and source_record.dag_id in (41,42,49,62,80,106,111,115,123,147,151,258,289,324,363,403,602,665,744,763,779,810,957,1580,1587)
;
...
where source_arm.project_id = 16
  and target_arm.project_id = 57
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (41,42,49,62,80,106,111,115,123,147,151,258,289,324,363,403,602,665,744,763,779,810,957,1580,1587) -- DAG ID to copy
...
where source_arm.project_id = 16
  ...
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (41,42,49,62,80,106,111,115,123,147,151,258,289,324,363,403,602,665,744,763,779,810,957,1580,1587) -- DAG IDs to copy
  ) and field_name = '__GROUPID__'
;
...
where source_role.role_name = 'Data Entry'
      ...
      and target_dag.project_id = 57
      and source_rights.group_id in (41,42,49,62,80,106,111,115,123,147,151,258,289,324,363,403,602,665,744,763,779,810,957,1580,1587)
;