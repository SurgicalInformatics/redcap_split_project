...
select target_dag.project_id,
    ...
    inner join redcap_data_access_groups target_dag on target_dag.group_name = source_dag.group_name
        where source_dag.project_id = 16
          and target_dag.project_id = 60
          and source_record.dag_id in (47,136,146,156,187,207,212,260,273,314,316,335,404,667,668,743,764,769,780,786,789,1597)
;
...
where source_arm.project_id = 16
  and target_arm.project_id = 60
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (47,136,146,156,187,207,212,260,273,314,316,335,404,667,668,743,764,769,780,786,789,1597) -- DAG ID to copy
...
where source_arm.project_id = 16
  ...
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (47,136,146,156,187,207,212,260,273,314,316,335,404,667,668,743,764,769,780,786,789,1597) -- DAG IDs to copy
  ) and field_name = '__GROUPID__'
;
...
where source_role.role_name = 'Data Entry'
      ...
      and target_dag.project_id = 60
      and source_rights.group_id in (47,136,146,156,187,207,212,260,273,314,316,335,404,667,668,743,764,769,780,786,789,1597)
;