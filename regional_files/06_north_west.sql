...
select target_dag.project_id,
    ...
    inner join redcap_data_access_groups target_dag on target_dag.group_name = source_dag.group_name
        where source_dag.project_id = 16
          and target_dag.project_id = 55
          and source_record.dag_id in (37,52,55,99,107,110,122,128,160,179,181,213,233,236,239,242,267,276,291,293,333,375,398,422,470,547,549,597,603,805)
;
...
where source_arm.project_id = 16
  and target_arm.project_id = 55
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (37,52,55,99,107,110,122,128,160,179,181,213,233,236,239,242,267,276,291,293,333,375,398,422,470,547,549,597,603,805) -- DAG ID to copy
...
where source_arm.project_id = 16
  ...
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (37,52,55,99,107,110,122,128,160,179,181,213,233,236,239,242,267,276,291,293,333,375,398,422,470,547,549,597,603,805) -- DAG IDs to copy
  ) and field_name = '__GROUPID__'
;
...
where source_role.role_name = 'Data Entry'
      ...
      and target_dag.project_id = 55
      and source_rights.group_id in (37,52,55,99,107,110,122,128,160,179,181,213,233,236,239,242,267,276,291,293,333,375,398,422,470,547,549,597,603,805)
;