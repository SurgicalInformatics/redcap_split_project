...
-- 'CCP UK SARI - North East and Yorkshire'
...
select target_dag.project_id,
    ...
    inner join redcap_data_access_groups target_dag on target_dag.group_name = source_dag.group_name
        where source_dag.project_id = 16
          and target_dag.project_id = 53
          and source_record.dag_id in (39,40,43,60,84,121,144,201,235,241,245,250,251,268,270,284,296,313,334,357,399,471,542,670,683,685,716,732,742,759,765,806,1345)
;
...
where source_arm.project_id = 16
  and target_arm.project_id = 53
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (39,40,43,60,84,121,144,201,235,241,245,250,251,268,270,284,296,313,334,357,399,471,542,670,683,685,716,732,742,759,765,806,1345) -- DAG ID to copy
...
where source_arm.project_id = 16
  ...
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (39,40,43,60,84,121,144,201,235,241,245,250,251,268,270,284,296,313,334,357,399,471,542,670,683,685,716,732,742,759,765,806,1345) -- DAG IDs to copy
  ) and field_name = '__GROUPID__'
;
...
where source_role.role_name = 'Data Entry'
      ...
      and target_dag.project_id = 53
      and source_rights.group_id in (39,40,43,60,84,121,144,201,235,241,245,250,251,268,270,284,296,313,334,357,399,471,542,670,683,685,716,732,742,759,765,806,1345)
;