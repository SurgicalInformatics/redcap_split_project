...
select target_dag.project_id,
    ...
    inner join redcap_data_access_groups target_dag on target_dag.group_name = source_dag.group_name
        where source_dag.project_id = 16
          and target_dag.project_id = 50
          and source_record.dag_id in (31,93,182,265,307,318,339,370,787,807,808,948,949,1198,2475)
;
...
where source_arm.project_id = 16
  and target_arm.project_id = 50
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (31,93,182,265,307,318,339,370,787,807,808,948,949,1198,2475) -- DAG ID to copy
...
where source_arm.project_id = 16
  ...
  and record.record in (
    select record from redcap_data
     where field_name = '__GROUPID__'
       and project_id = source_arm.project_id
       and value in (31,93,182,265,307,318,339,370,787,807,808,948,949,1198,2475) -- DAG IDs to copy
  ) and field_name = '__GROUPID__'
;
...
where source_role.role_name = 'Data Entry'
      ...
      and target_dag.project_id = 50
      and source_rights.group_id in (31,93,182,265,307,318,339,370,787,807,808,948,949,1198,2475)
;